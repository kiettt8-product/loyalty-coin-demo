...
if (await page.getByRole("heading", { name: "Display Users" }).count() !== 2) throw new Error("Each reward must include its own Display Users section");
if (await page.getByRole("heading", { name: "Reward Active Time" }).count() !== 2) throw new Error("Each reward must include its own Active Time section");
if (await page.getByRole("heading", { name: "Apply Limit" }).count() !== 2) throw new Error("Each reward must include its own Apply Limit section");
