...
if (await page.getByRole("heading", { name: "Display Users" }).count() !== 1) throw new Error("New rewards must share Rule Set A by default");
if (new Set(await page.locator(".promo-reward-rule-set").evaluateAll(selects => selects.map(select => select.value))).size !== 1) throw new Error("New rewards must reference the same Rule Set by default");
...
await page.locator("#promoAddRuleSet").click();
await page.locator(".promo-reward-rule-set").nth(1).selectOption({ label: "Rule Set B" });
if (await page.getByRole("heading", { name: "Display Users" }).count() !== 2) throw new Error("Custom Rule Set must create an independent full rules section");
...
if (await page.locator(".promo-reward-item").count() !== 2 || await page.locator(".promo-rule-set-config").count() !== 2) throw new Error("Submitted rewards and Rule Sets must persist");
if (await page.locator("#promoRecurringPeriod-2").inputValue() !== "Recur Weekly") throw new Error("Rule Set B recurring period must persist independently");
if (await page.locator("#promoRecurringWeekStart-2").inputValue() !== "Tuesday") throw new Error("Rule Set B recurring detail must bind to the correct Rule Set");
